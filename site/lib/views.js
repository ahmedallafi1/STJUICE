...
function quoteTotals(quote) {
  ...
  const liveQuote = quote.mode === "production";
  return `<div class="quote-totals">${rows.map(([label, value]) => `<div><span>${escapeHtml(label)}</span><strong>${value < 0 ? `−${money(Math.abs(value))}` : money(value)}</strong></div>`).join("")}<div class="quote-totals__grand"><span>${liveQuote ? "Total" : "Preview total"}</span><strong>${money(quote.totals.total.amount)}</strong></div>${liveQuote ? "" : `<p class="quote-totals__note">Tax and any delivery/service fees will be finalized when online ordering goes live.</p>`}</div>`;
}
...
function checkoutPayment(state) {
  const checkout = state.checkout;
  const liveCommerce = state.runtimeConfig?.mode === "production" && state.runtimeConfig?.payments?.live === true;
...
function renderOrder(orderId, { state }) {
  ...
  const liveOrder = order?.mode === "production";
  if (state.orderLoading || !order || order.id !== orderId) return `${pageHero("ORDER STATUS", "Loading your order…", "Checking the latest status.")}<section class="section section--cream"><div class="container"><div class="skeleton skeleton--line"></div></div></section>`;
